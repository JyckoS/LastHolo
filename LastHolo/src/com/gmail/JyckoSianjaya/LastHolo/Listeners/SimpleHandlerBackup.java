...
public class SimpleHandlerBackup {
	...
	public static SimpleHandlerBackup getInstance() {
		if (instance == null) instance = new SimpleHandlerBackup();
		return instance;
	}
    ...
    if (e.isCancelled()) return;
		...
		if (p.hasPermission("holo.chat")) {
			...
			if (p.hasPermission("holo.color")) {
				...
				if (cache.getHoloData(uuid) == null) {
					synchronized(this) {
					holo = HologramsAPI.createHologram(LastHolo.getInstance(), locas);
					}
						...
						if (holo.size() > 0) {
							...
							for (int i = 0; i <= size; i++) {
								...
								for (LineData ld : info.values()) {
									...
									holo.insertTextLine(line, finals);
								}
							}
							if (data.isChatPopping() == true) {
								holo.teleport(locas);
							}
							holodata.setLiveTicks(data.getHoloLiveTick() + size * 20);
							holodata.setHeight(holodata.getHeight() + size * 0.18);
